<?php
// File: modules/keycloak-account-pages/includes/class-keycloak-account-pages-rest.php

if (!defined('ABSPATH')) exit;

class Keycloak_Account_Pages_Rest {
  ...
  public static function get_connections(WP_REST_Request $req) {
    ...
    foreach ($active as $row) {
      // Ignorer le provider '_keycloak' s'il existe encore (nettoyage progressif)
      if ($row['provider_slug'] !== '_keycloak') {
        $map[$row['provider_slug']] = $row;
      }
    }

    // 2️⃣ Synchroniser avec Keycloak pour avoir les données à jour (utilise la méthode unifiée)
    $kc_user_id = Keycloak_Account_Pages_Keycloak::get_kc_user_id_for_wp_user($user_id);
    if ($kc_user_id && function_exists('admin_lab_sync_keycloak_federated_identities')) {
      // Utiliser la méthode unifiée (API Admin Keycloak en priorité, claims en fallback)
      admin_lab_sync_keycloak_federated_identities($user_id, $kc_user_id);
      
      // Recharger les connexions depuis la BDD après synchronisation
      $active = Admin_Lab_DB::getInstance()->get_active_keycloak_connections($user_id);
      $map = [];
      foreach ($active as $row) {
        // Ignorer le provider '_keycloak' s'il existe encore (nettoyage progressif)
  ...
  public static function connect_provider(WP_REST_Request $req) {
    ...
    try {
      ...
      if (empty($kc_user_id)) {
        $kc_user_id = Keycloak_Account_Pages_Keycloak::ensure_user_exists($user_id);
      }
      
      // S'assurer que le rôle manage-account-links est assigné
      if (!empty($kc_user_id)) {
        try {
          Keycloak_Account_Pages_Keycloak::assign_account_role($kc_user_id, 'manage-account-links');
        } catch (Exception $e) {
        ...
        // Note: On ne stocke plus le kc_user_id avec provider_slug='_keycloak'
        // Le kc_user_id est maintenant stocké via keycloak_identity_id dans les connexions réelles
      }
    } catch (Exception $e) {
      if (defined('WP_DEBUG') && WP_DEBUG && function_exists('error_log')) {
  ...
  public static function disconnect_provider(WP_REST_Request $req) {
    ...
    if ($is_last_provider) {
      ...
      $has_password = Keycloak_Account_Pages_Keycloak::user_has_password($kc_user_id);
    ...
    try {
      Keycloak_Account_Pages_Keycloak::unlink_provider($kc_user_id, $kc_alias);
  ...
  public static function get_profile(WP_REST_Request $req) {
    ...
    if ($kc_user_id) {
      try {
        ...
        $profile['has_keycloak_password'] = Keycloak_Account_Pages_Keycloak::user_has_password($kc_user_id);
      } catch (Exception $e) {
  ...
  public static function init_password_change(WP_REST_Request $req) {
    ...
    try {
      ...
      $auth_url = Keycloak_Account_Pages_Keycloak::build_reauth_url($state);
  ...
  public static function finalize_password_change(string $token): array {
    ...
    try {
      ...
      Keycloak_Account_Pages_Keycloak::reset_password($kc_user_id, $new_password, false);
  ...
  public static function update_password(WP_REST_Request $req) {
    ...
    try {
      ...
      Keycloak_Account_Pages_Keycloak::reset_password($kc_user_id, $pass1, false);
  ...
  public static function init_email_change(WP_REST_Request $req) {
    ...
    try {
      ...
      $existing_user = Keycloak_Account_Pages_Keycloak::find_user_by_email($new_email);
  ...
  public static function finalize_email_change(string $token): array {
    ...
    try {
      ...
      Keycloak_Account_Pages_Keycloak::update_user_safe($kc_user_id, [
        'email' => $new_email,
      ...
      try {
        ...
        try {
          ...
          Keycloak_Account_Pages_Keycloak::send_verification_email($kc_user_id, $client_id ?: null, $redirect_uri ?: null);
        } catch (Exception $e2) {
  ...
  public static function keycloak_callback(WP_REST_Request $req) {
    ...
    if ($kap_token && empty($state_param)) {
      ...
      if ($kc_user_id) {
        try {
          $providers = Keycloak_Account_Pages_Keycloak::get_providers();
          $kc_alias = $providers[$provider]['kc_alias'] ?? $provider;
          $fed = Keycloak_Account_Pages_Keycloak::get_federated_identities($kc_user_id);
    ...
    if ($error) {
      ...
      if ($error === 'invalid_redirect_uri') {
        $redirect_uri = (string) Keycloak_Account_Pages_Keycloak::opt('kc_redirect_uri');
        $error_message = sprintf(
          ...
          esc_html(Keycloak_Account_Pages_Keycloak::opt('kc_client_id'))
        );
      }
    ...
    if (!empty($code)) {
      try {
        ...
        $token_response = Keycloak_Account_Pages_Keycloak::exchange_code_for_token($code, $code_verifier ?: '');
        $access_token = $token_response['access_token'] ?? '';
        
        if (!empty($access_token)) {
          $kc_user_id_from_token = Keycloak_Account_Pages_Keycloak::get_user_id_from_token($access_token);
          ...
          if (empty($kc_user_id) && !empty($kc_user_id_from_token)) {
            ...
            // Note: Le kc_user_id est stocké via keycloak_identity_id dans les connexions réelles
            // Plus besoin de stocker avec provider_slug='_keycloak'
          }
        }
        ...
        if (!empty($st['mode']) && $st['mode'] === 'broker_link') {
          ...
          $issued_for = (string) Keycloak_Account_Pages_Keycloak::opt('kc_client_id');
          ...
          $broker_url = Keycloak_Account_Pages_Keycloak::build_broker_link_url(
            $kc_alias,
            $redirect_uri,
            $session_state,
            $issued_for
    ...
    // Si pas de kc_user_id, on ne peut pas enregistrer de connexion
    // La synchronisation se fera automatiquement lors de la prochaine connexion
