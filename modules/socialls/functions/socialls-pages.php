<?php
// File: modules/socialls/functions/socialls-pages.php

if (!defined('ABSPATH')) exit;

function admin_lab_socialls_create_pages() {
    $default_pages = [
        'socials' => [
            'title'   => 'Social Networks',
            'content' => '[socials_dashboard]'
        ]
    ];

    foreach ($default_pages as $slug => $data) {
        $option_key = 'socialls_page_' . $slug;
...
function admin_lab_delete_socialls_pages() {
    $page_slugs = ['socials'];
...
function admin_lab_protect_socialls_pages() {
    $page_id = get_option('socialls_page_socials');
